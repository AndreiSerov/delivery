...
class Location private constructor(
    ...
    fun countStepsToOtherLocation(location: Location): Either<LocationError, Int> = either { abs(x - location.x) + abs(y - location.y) }

    fun takeAStep(
        location: Location,
        count: Int = 1,
    ): Location = takeAStepHelper(x, y, location, count)
    ...
    private tailrec fun takeAStepHelper(
        x: Int,
        y: Int,
        location: Location,
        count: Int,
